...
namespace Microsoft.Research.Science.FetchClimate2.Tests
{
    ...
    public class CpcTests
    {
        ...
        {
            var storage = TestDataStorageFactory.GetStorage(TestConstants.UriCpc);
            CPCDataSource.CpcDataHandler handler = await CPCDataSource.CpcDataHandler.CreateAsync(storage);

            TimeRegion tr = new TimeRegion(firstYear:1949,lastYear:1949).GetMonthlyTimeseries(firstMonth: 4, lastMonth: 4);//data index 15
