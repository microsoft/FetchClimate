...
D3.ColorPalette.RGBtoHSL = function (rgbaColor) {
    ...
    _l = maxcolor;// (maxcolor + mincolor) / 2.0;

    if (maxcolor == 0)
        _s = 0.0;
    else
        _s = (maxcolor - mincolor) / maxcolor;
    /*{
        if (_l < 0.5)
...
D3.ColorPalette.HSLtoRGB = function (hslaColor) {
    ...
    var c = lightness * saturation; //(1.0 - Math.abs(2.0 * lightness - 1.0)) * saturation;
    ...
    var m = 255 * (lightness - c); //(lightness - c / 2.0) * 255;
    var temp = _r + m;
    if (temp > 255) _r = 255;
    else if (temp < 0) _r = 0;
...
D3.Markers = function (div, master) {
    ...
    this.draw = function (data) {
        ...
        if (_colorPalette && _colorPalette.isNormalized) {
            _colorRange = D3.Utils.getMinMax(_data.color);
        }
        if (_sizePalette && _sizePalette.isNormalized) {
            _sizeRange = D3.Utils.getMinMax(_data.size);
        }
    ...
    this.computeLocalBounds = function (step, computedBounds) {
        var dataToPlotX = this.xDataTransform && this.xDataTransform.dataToPlot;
        var dataToPlotY = this.yDataTransform && this.yDataTransform.dataToPlot;
        return D3.Utils.getBoundingBoxForArrays(_data.x, _data.y, dataToPlotX, dataToPlotY);
    }
    ...
    this.getLocalPadding = function () {
        ...
        if (size instanceof Array) {
            if (_sizePalette)
                ...
                padding = D3.Utils.getMinMax(size).max / 2;
            }
        }
        else {
            padding = size / 2;
    ...
    this.renderCore = function (plotRect, screenSize) {
        ...
        if (drawBasic) {
            ...
            for (var prop in markersData) {
                ...
                markersData[prop] = { value: v, isArray: v instanceof Array };
            }
    ...
    this.findToolTipMarkers = function (xd, yd, xp, yp) {
        ...
        var sizeIsArray = localSize instanceof Array;
        var colorIsArray = _data.color instanceof Array;
        var sizePaletteNormalized = _sizePalette && _sizePalette.isNormalized;
        ...
        if (typeof (_shape) == "object") {
            if (typeof (_shape.hitTest) == "function") {
                ...
                for (var i = 0; i < n; i++) {
                    ...
                    for (var prop in _data) {
                        var v = _data[prop];
                        if (v instanceof Array) drawData[prop] = v[i];
    ...
    this.getLegend = function () {
        ...
        var refreshSize = function () {
            sizeIsArray = _data.size instanceof Array;
            if (sizeIsArray) {
                size = maxSize;
                if (_sizePalette) {
                    if (sizeIsVisible == 0) {
        ...
        var refreshColor = function () {
            colorIsArray = _data.color instanceof Array;
            drawBorder = false;
...
D3.AdaptMarkerSize = function (markers, plotRect, screenSize, transform, context) {
...
D3.heatmapBackgroundRenderer = new D3.SharedRenderWorker("scripts/external/d3heatmapworker.js",
